            ...
            <div class="divide-y divide-gray-100">
                ...
                <div class="px-6 py-3 flex items-center justify-between gap-4">
                    <div class="min-w-0">
                        ...
                        @endphp
                        <div class="text-xs font-semibold mt-0.5 {{ $balance > 0 ? 'text-green-700' : ($balance < 0 ? 'text-red-700' : 'text-gray-500') }}">
                        ...
                        <div class="text-[11px] text-gray-400">Inclut automatiquement les trop-perçus.</div>
                        ...
                        <details class="text-xs text-gray-500 mt-1">
                            <summary class="cursor-pointer text-indigo-600 hover:text-indigo-800">
                                Historique ({{ $balanceHistory->count() }})
                            </summary>
                    ...
                    <div class="flex items-center gap-3 shrink-0 flex-wrap justify-end">
                        ...
                        @if($canEdit)
                        <form method="POST" action="{{ route('admin-local.tontines.participants.balance', [$tontine, $p]) }}"
                              class="flex items-center gap-1">
                            @csrf @method('PATCH')
                            <label for="balance-{{ $p->id }}" class="sr-only">Solde de {{ $p->full_name }}</label>
                            <input id="balance-{{ $p->id }}" type="number" name="balance" step="0.01"
                            ...
                            <input type="text" name="reason" maxlength="500" placeholder="Motif"
                                   class="w-28 text-xs border border-gray-300 rounded px-1.5 py-1">
                            <button type="submit" class="text-xs bg-indigo-50 hover:bg-indigo-100 text-indigo-700 px-2 py-1 rounded">Solde</button>
                        </form>
                        @endif
                    </div>
                </div>
