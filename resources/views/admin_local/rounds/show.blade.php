@extends('layouts.app')
@section('title', ($round->isPreliminary() ? 'Préliminaire' : 'Tour').' #'.$round->round_number)
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-sm text-gray-500 mb-1">
                ...
                <span>/</span>
                <span>{{ $round->isPreliminary() ? 'Préliminaire' : 'Tour' }} #{{ $round->round_number }}</span>
            </div>
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-bold text-gray-900">
                    {{ $round->isPreliminary() ? 'Tour préliminaire' : 'Tour' }} #{{ $round->round_number }}
                </h1>
        ...
        <div class="flex gap-2 flex-wrap">
            ...
            <form method="POST" action="{{ route('admin-local.rounds.reopen', [$tontine, $round]) }}"
            ...
            <form method="POST" action="{{ route('admin-local.rounds.draw', [$tontine, $round]) }}"
            ...
            <form method="POST" action="{{ route('admin-local.rounds.cancel-draw', [$tontine, $round]) }}"
    ...
    <div class="{{ $round->isPreliminary() ? 'max-w-2xl' : 'grid grid-cols-1 lg:grid-cols-2 gap-8' }}">
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <div class="px-6 py-3 border-b border-gray-50 last:border-0">
                ...
                <div class="flex gap-4 text-xs text-gray-500 mb-2 flex-wrap">
                    ...
                    @if($payment->due_date)
                        ...
                        @if($payment->status === 'paid' && $payment->paid_at)
                            <span>Payé le <strong>{{ $payment->paid_at->format('d/m/Y') }}</strong></span>
                        @endif
                        @if($daysLate > 0)
                            <span class="text-red-600 font-medium">
                                {{ $daysLate }}j de retard
                                @if($penalty > 0) — Pénalité : {{ number_format($penalty, 2) }} € @endif
                            </span>
                        @endif
                    @endif
                </div>
                ...
                <form method="POST" action="{{ route('admin-local.rounds.payments.update', [$tontine, $round, $payment]) }}"
